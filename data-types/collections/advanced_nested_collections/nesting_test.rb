...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_dishes_names_for_olive_garden
    #=======================
    dishes_names = []
    dish_names = stores[:olive_garden][:dishes].each do |dish|
  ...
  def test_list_of_employees_across_all_restaurants
    employee_names = []
    #=======================
    all_employees = stores.each do |k,v|
      v[:employees].each do |name|
        employee_names << name
      end
    end
    #=======================
    assert_equal ["Jeff","Zach","Samantha","Bob","Sue","James","Alvin","Simon","Theodore"], employee_names.concat
  end
  ...
  def test_full_menu_price_for_olive_garden
    #=======================
    full_menu_price = 0
    stores[:olive_garden][:dishes].each do |k|
      # require "pry"; binding.pry
      full_menu_price = full_menu_price + k[:price]
    end
  ...
  def test_full_menu_for_olive_garden
    #=======================
    olive_garden_menu = {}
    dish_name = stores[:olive_garden][:dishes].each do |dish|
       stores[:olive_garden][:dishes].each do |single|
         # require "pry"; binding.pry
         if dish[:name] != single[:name]
         else
           olive_garden_menu = olive_garden_menu.merge({dish[:name] => single})
         end
       end
    end
    #=======================
     ...
     stores.each do |k,v|
       v[:dishes].each do |dish|
         ...
         if dish[:name] != dish_name
           ...
           full_menu = full_menu.merge({dish[:name] => dish})
         end
       end
     end
    #=======================
